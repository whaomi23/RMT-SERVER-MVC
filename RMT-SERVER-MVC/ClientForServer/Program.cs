...
namespace RMT_CLIENT_MVC
{
    public class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            // Inicializar RemoteMonitor
            RemoteMonitor.Initialize("https://192.168.1.107:7277");
        ...
        static async Task RegisterWithServer()
        {
            var content = new FormUrlEncodedContent(new[]
            {
                ...
                new KeyValuePair<string, string>("driveL", string.Join("; ", systemInfo.Drives.ToString()))
            });
                    ...
                    foreach (var cmd in jsonResponse.commands)
                    {
                        ...
                        else if (commandText.Equals("SCREEN-CAP", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            result = ("Captura de pantalla tomada y enviada al servidor", false);
                                 ...
                                 commandText.Equals("CLOSE", StringComparison.OrdinalIgnoreCase) ||
        ...
        static (string output, bool isError) ExecuteCommand(string command)
        {
            try
            {
                ...
                else if (command.StartsWith("GET_DIR_FILE"))
                {
                    var filePath = command.Substring("GET_DIR_FILE".Length).Trim('"');
                    return GetDirFile(filePath);
                }
        ...
        static async Task<(string output, bool isError)> GetDirFile(string filePath)
        {
            try
            {
                if (!File.Exists(filePath))
                ...
                content.Add(new StringContent(base64File), "fileData");
                content.Add(new StringContent(filePath), "sourcePath");

                // Enviar el archivo al servidor
                var response = await httpClient.PostAsync($"{serverURL}/ReceiveFileFromClient", content);
                response.EnsureSuccessStatusCode();
                ...
                if (jsonResponse.success == true)
                {
                    return ($"Archivo enviado correctamente: {Path.GetFileName(filePath)}", false);
                }
                else
                {
                    return ($"Error al enviar archivo: {jsonResponse.message}", true);
                }
            }
            catch (Exception ex)
            {
                return ($"Error al procesar archivo: {ex.Message}", true);
            }
        }
